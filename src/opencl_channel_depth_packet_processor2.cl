...
void encodeChannels16moduloSingelHyp(float16* channels_1, float phase_hyp, uint channel_count)
{
 	uint num_channels = NUM_CHANNELS;
	float num_channels_f = (float)(num_channels);
	float cwidth=M_PI_F/3.0f;
	...
	//initialize channel vector for channels 1-16
	float16 cpos_f = (float16)(1.0f,2.0f,3.0f,4.0f,5.0f,6.0f,7.0f,8.0f,9.0f,10.0f,11.0f,12.0f,13.0f,14.0f,15.0f,16.0f)+(float16)(16.0f)*channel_count;
...
void kernel processPixelStage2_phase_channels(global const float3 *a_in, global const float3 *b_in, global float *phase_1, global float *phase_2, global float *ir_sums, global float16* channels_1, global float16* channels_2, global float16* channels_3, global float16* channels_4, global float16* channels_1_phase_1, global float16* channels_2_phase_1, global float16* channels_3_phase_1, global float16* channels_4_phase_1, global float16* channels_1_phase_2, global float16* channels_2_phase_2,  global float16* channels_3_phase_2, global float16* channels_4_phase_2, constant float* camera_intrinsics)
{
  const uint i = get_global_id(0);
  float3 a = a_in[i];
  float3 b = b_in[i];
  ...
  //}
	...
	float16 ch_1_phase1, ch_2_phase1, ch_1_phase2, ch_2_phase2, ch_3_phase1, ch_4_phase1, ch_3_phase2, ch_4_phase2;
	encodeChannels16moduloSingelHyp(&ch_1_phase1, phase_first, 0);
	if(NUM_CHANNELS>16)
		encodeChannels16moduloSingelHyp(&ch_2_phase1, phase_first, 1);
	if(NUM_CHANNELS>32)
		encodeChannels16moduloSingelHyp(&ch_3_phase1, phase_first, 2);
	if(NUM_CHANNELS>48)
		encodeChannels16moduloSingelHyp(&ch_4_phase1, phase_first, 3);

	encodeChannels16moduloSingelHyp(&ch_1_phase2, phase_second, 0);
	if(NUM_CHANNELS>16)
		encodeChannels16moduloSingelHyp(&ch_2_phase2, phase_second, 1);
	if(NUM_CHANNELS>32)
		encodeChannels16moduloSingelHyp(&ch_3_phase2, phase_second, 2);
	if(NUM_CHANNELS>48)
		encodeChannels16moduloSingelHyp(&ch_4_phase2, phase_second, 3);

	channels_1[i] = w_err1*ch_1_phase1+w_err2*ch_1_phase2;
	channels_2[i] = w_err1*ch_2_phase1+w_err2*ch_2_phase2;
	channels_3[i] = w_err1*ch_3_phase1+w_err2*ch_3_phase2;
	channels_4[i] = w_err1*ch_4_phase1+w_err2*ch_4_phase2;
	...
	channels_3_phase_1[i] = ch_3_phase1;
	channels_4_phase_1[i] = ch_4_phase1;
	channels_3_phase_2[i] = ch_3_phase2;
	channels_4_phase_2[i] = ch_4_phase2;
...
void filter_channels16_help(global const float16* channels, float16* channels_filtered, const uint i, global const float* gauss_filt_array)
{
  ...
  // Filter kernel
    sumx=(float16)(0.0f);
		...
		*channels_filtered = sumx/sum_gauss;
    //channels_filtered[(loadY*512+loadX)] = sumx/divby;
  }
...
void kernel filter_channels(global const float16* channels_1, global const float16* channels_2, global const float16* channels_3, global const float16* channels_4, global const float* gauss_filt_array, global const float *phase_1, global const float *phase_2, global const float16* channels_1_phase_1, global const float16* channels_2_phase_1, global const float16* channels_3_phase_1, global const float16* channels_4_phase_1, global const float16* channels_1_phase_2, global const float16* channels_2_phase_2, global const float16* channels_3_phase_2, global const float16* channels_4_phase_2, global const float *x_table, global const float *z_table, global float* depth)
{
	const uint i = get_global_id(0);
	float16  channels_filtered_1,  channels_filtered_2,  channels_filtered_3,  channels_filtered_4;
	...
	if(NUM_CHANNELS>32)
	{
		filter_channels16_help(channels_3, &channels_filtered_3, i, gauss_filt_array);
	}
	if(NUM_CHANNELS>48)
	{
		filter_channels16_help(channels_4, &channels_filtered_4, i, gauss_filt_array);
	}

	float16 ch1,ch2,ch3,ch4;
	...
	ch3 = channels_3_phase_1[i];
	ch4 = channels_4_phase_1[i];
	float val1 = DOT_16(ch1,channels_filtered_1)+(NUM_CHANNELS > 16 ? DOT_16(ch2,channels_filtered_2): 0.0f)+(NUM_CHANNELS > 32 ? DOT_16(ch3,channels_filtered_3): 0.0f)+(NUM_CHANNELS > 48 ? DOT_16(ch4,channels_filtered_4): 0.0f);
	//encodeChannels32moduloSingelHyp(&ch1, &ch2, phase_second);
	ch1 = channels_1_phase_2[i];
	ch2 = channels_2_phase_2[i];
	ch3 = channels_3_phase_2[i];
	ch4 = channels_4_phase_2[i];
	float val2 = DOT_16(ch1,channels_filtered_1)+(NUM_CHANNELS > 16 ? DOT_16(ch2,channels_filtered_2): 0.0f)+(NUM_CHANNELS > 32 ? DOT_16(ch3,channels_filtered_3): 0.0f)+(NUM_CHANNELS > 48 ? DOT_16(ch4,channels_filtered_4): 0.0f);
  ...
  //depth[i] = max_val >= 0.05f ? DEPTH_SCALE*depth_linear: 0.0f;
...
void kernel processPixelStage2_depth_channels3(global const float *phase_1, global const float *phase_2, global const float *phase_3, global const float16* channels_filtered_1, global const float16* channels_filtered_2, global const float *x_table, global const float *z_table, global float* depth)
{
  ...
  float d = cond1 ? depth_fit : depth_linear; // r1.y -> later r2.z
	max_val = depth_linear < MIN_DEPTH || depth_linear > MAX_DEPTH ? 0.0f: max_val;
  depth[i] = max_val >= 0.12 ? DEPTH_SCALE*depth_linear: 0.0f;
}*/
